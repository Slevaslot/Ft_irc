...
private:
	...
	int _currentClient;
	std::vector<Client>::iterator _currentIt;
	int _cmdSize;
	bool clicli[2048];
	int _c;
...
public:
	...
	void nickCmd(std::vector<std::string> cmdSplited, Client &cli);
	...
	void ping(std::vector<std::string> cmdSplited, int fd, std::string nickname, std::string username);
	// void join(std::vector<std::string> cmdSplited, int fd, Client &currentClient);
	void join(std::vector<std::string> cmdSplited, int fd, Client &currentClient, int curr);
	...
	std::vector<Client>::iterator GetItClient(int fd);
	...
	Channel *getChannelPtr(std::string channelName);
	bool isChannelValid(std::string channelName);
