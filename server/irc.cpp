...
void Server::parse_exec_cmd(std::string cmd, int fd)
{
	...
	case (JOIN):
		join(cmdSplited, fd, *_currentIt, _currentClient);
		break;
	case (LIST):
		listChannels(fd);
		break;
	...
	case (TOPIC):
		if (_cmdSize >= 3)
			topicChannel(fd, cmdSplited[1], cmdSplited[2]);
		break;
	case (PART):
		if (_cmdSize >= 3)
	...
	case (PRIVMSG):
		if (_cmdSize >= 3)
			ping(cmdSplited, fd, clients[_currentClient].GetNickname(), clients[_currentClient].GetUsername());
		break;
	case (PING):
		if (_cmdSize >= 2)
			ping(cmdSplited, fd, clients[_currentClient].GetNickname(), clients[_currentClient].GetUsername());
