...
std::vector<Client>::iterator Server::GetItClient(int fd)
{
	int i = -1;
	std::vector<Client>::iterator it;
	for (it = clients.begin(); it < clients.end(); it++)
...
void Server::ClearClients(int fd)
{
	if (!channels.empty())
	{
		for (std::vector<Channel>::iterator it = channels.begin(); it < channels.end(); it++)
		{
			...
			{
				for (std::vector<Client>::iterator it1 = it->GetClients().begin(); it1 < it->GetClients().end(); it1++)
				{
					it->GetClients().erase(it1);
					if (it->GetClients().size() <= 1)
						ClearClients(fd);
				}
			}
