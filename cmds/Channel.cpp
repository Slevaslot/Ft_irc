...
void Channel::sendMsgAllClientsEx(std::string msg, int fd, Client &sender)
{
	for (size_t i = 0; i < _clients.size(); i++)
	{
		if (_clients[i].GetFd() != fd)
		{
			std::string privMsg = ":" + sender.GetNickname() + "!" + sender.GetUsername() + "@localhost PRIVMSG " + _name + " :" + msg + "\r\n";
			send(_clients[i].GetFd(), privMsg.c_str(), privMsg.size(), 0);
		}
	}
}
